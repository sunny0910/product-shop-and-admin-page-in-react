...
import { TableHead, TableBody, TableCell, TableRow, Paper, IconButton, Tooltip, CircularProgress, LinearProgress, Snackbar, TableSortLabel, TablePagination } from '@material-ui/core';
...
class DataTable extends Component
{
    ...
    mainCheckboxOnChange(e) {
        this.setState((state, props) => ({
            selected: (state.mainCheckBoxSelected)? [] : props.users.map(row => row.id),
    ...
    render() {
        ...
        return (
                <Paper className="data-table">
                    { (this.state.deleteNotification) ? <Snackbar autoHideDuration={1500} anchorOrigin={anchorOrigin} open onClose={this.hideDeleteNotification} message="User Deleted!"/> : ''}
                    ...
                    {this.state.spinnerLoading ?
                        <CircularProgress color="primary" variant="indeterminate" className="loader"/> :
                                        ...
                                        {this.stableSort(this.props.users).map((user, index) => {
                                            ...
                                            return (
                                                <TableRow key={index} hover selected={isSelected} >
                                                    <TableCell>
                                                        <CheckBox selected={isSelected} onClick={event => this.checkboxOnChange(event,user.id) } color={(isSelected) ? "secondary": "inherit"} />
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Link to={user.url.view} className="actions">
                                                            <Tooltip title="View" enterDelay={300} placement={"left"} >
                                                    ...
                                                    <TableCell>
                                                        <Link to={user.url.edit} className="actions">
                                                            <Tooltip title="Edit" enterDelay={300} placement={"left"} >
                                                    ...
                                                    <TableCell>
                                                        <Tooltip title="Delete" enterDelay={300} placement={"left"} >
                                                            <div>
                                                                <IconButton disabled = {this.props.userId === user.id} aria-label="Delete" onClick= {() => this.deleteRow(user.id)} >
                                                                    <Delete/>
                                                                </IconButton>
                                                            </div>
                                                        </Tooltip>
                            ...
                            <TablePagination component="div" count={this.props.users.length} rowsPerPage={this.state.rowsPerPage} page={this.state.page} onChangePage={this.handlePageChange} onChangeRowsPerPage={this.handleChangeRowsPerPage} />
                        </React.Fragment>
                        )
                    }
            </Paper>
